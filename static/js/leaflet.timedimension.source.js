...
L.TimeDimension = (L.Layer || L.Class).extend({
    ...
    _generateAvailableTimes: function () {
        if (this.options.times) {
            ...
            var period = this.options.period || 'P3D';
...
L.TimeDimension.Util = {
    ...
    parseAndExplodeTimeRange: function(timeRange, overwritePeriod) {
        ...
        var period = (tr.length > 2 && tr[2].length) ? tr[2] : "P3D";
...
L.TimeDimension.Layer.GeoJson = L.TimeDimension.Layer.extend({
    ...
    initialize: function(layer, options) {
        L.TimeDimension.Layer.prototype.initialize.call(this, layer, options);
        this._updateTimeDimension = this.options.updateTimeDimension || false;
        this._updateTimeDimensionMode = this.options.updateTimeDimensionMode || 'replace'; // 'union', 'replace' or extremes
...
/*
 * L.TimeDimension.Layer.HexGrid:
 */

L.TimeDimension.Layer.HexGrid = L.TimeDimension.Layer.extend({
    ...
    _update: function() {
        ...
        // new coordinates:
        var layer = L.hexGrid(null, this._baseLayer.options);
        var layers = this._baseLayer.getLayers();
        for (var i = 0, l = layers.length; i < l; i++) {
            var feature = this._getFeatureBetweenDates(layers[i].feature, minTime, maxTime);
            if (feature) {
...
L.timeDimension.layer.hexGrid = function(layer, options) {
    return new L.TimeDimension.Layer.HexGrid(layer, options);
};
