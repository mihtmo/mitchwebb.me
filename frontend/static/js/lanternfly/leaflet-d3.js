...
(function (global, factory) {
	...
	L.HexbinLayer = L.SVG.extend({
		 ...
		 */
		_hideHex: function() {
			document.documentElement.style.setProperty('--hex-opacity', '0.0');
		},
        ...
        /**
          ...
          */
        _showHex: function() {
            document.documentElement.style.setProperty('--hex-opacity', '0.9');
        },
        ...
        // TODO: Make proper logarithmic scale

		_linearlySpace: function(from, to, length) {
			var arr = new Array(length);
			var step = (to - from) / Math.max(length - 1, 5);
	...
	L.HexbinHoverHandler = {
		...
		tooltip: function(options) {
			...
			return {
				mouseover: function (layer_hexgrid, data) {
					var event = d3.event;
					var gCoords = d3.mouse(this);

					tooltip
						.style('visibility', 'visible')
						.html(options.tooltipContent(data, layer_hexgrid));
				...
				mouseout: function (layer_hexgrid, data) {
					tooltip
						.style('visibility', 'hidden')
						.html();
				}
			...
			return {
				mouseover: function (layer_hexgrid, data) {
					...
					o.select('path.hexbin-hexagon')
						.attr('d', function (d) {
							return layer_hexgrid._hexLayout.hexagon(layer_hexgrid.options.radius);
						});
				},
				mouseout: function (layer_hexgrid, data) {
					var o = d3.select(this.parentNode);
					o.select('path.hexbin-hexagon')
						.attr('d', function (d) {
							return layer_hexgrid._hexLayout.hexagon(layer_hexgrid._scale.radius(layer_hexgrid._fn.radiusValue.call(layer_hexgrid, d)));
						});
				}
			};
		...
		resizeScale: function(options) {
			...
			return {
				mouseover: function (layer_hexgrid, data) {
					...
					o.select('path.hexbin-hexagon')
						.attr('d', function (d) {
							return layer_hexgrid._hexLayout.hexagon(layer_hexgrid._scale.radius.range()[1] * (1 + options.radiusScale));
						});
				},
				mouseout: function (layer_hexgrid, data) {
					var o = d3.select(this.parentNode);
		...
		compound: function(options) {
			...
			return {
				mouseover: function (layer_hexgrid, data) {
					var that = this;
					options.handlers.forEach(function(h) { h.mouseover.call(that, layer_hexgrid, data); });
				},
				mouseout: function (layer_hexgrid, data) {
					var that = this;
					options.handlers.forEach(function(h) { h.mouseout.call(that, layer_hexgrid, data); });
				}
			};
